...
if __name__ == "__main__":
    ...
    references= pd.read_csv("lyrics_testingdata.csv")
    sentences = pd.read_csv("bigram_result.csv")
    '''
    ...
    sentence = [x.split() for x in sentences.text.to_list() if type(x) !=float ]
    msd = MultisetDistances(references=references.text)
